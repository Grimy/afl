...
static s8  interesting_8[]  = { '\033' };
static s16 interesting_16[] = { '[\033', ']\033', '99' };
static s32 interesting_32[] = { ';;;;', '9999', 'r9[\033' };
...
static void maybe_add_auto(u8* mem, u32 len) {
  ...
  if (len == 2) {
    ...
    while (i--) 
      if (*((u16*)mem) == interesting_16[i]) return;
  ...
  if (len == 4) {
    ...
    while (i--) 
      if (*((u32*)mem) == interesting_32[i]) return;
...
static u8 fuzz_one(char** argv) {
  ...
  u8  *in_buf, out_buf[MAX_FILE], *orig_in, *eff_map = 0;
  ...
  if (pending_favored) {
    ...
    if ((queue_cur->was_fuzzed || !queue_cur->favored) && UR(100) < 99)
            return 1;

  } else if (!dumb_mode && queue_cur->was_fuzzed && queued_paths > 10) {

    /* Otherwise, still possibly skip non-favored cases, albeit less often.
       The odds of skipping stuff are higher for already-fuzzed inputs and
       lower for never-fuzzed entries. */

      if (UR(100) < (queue_cur->favored ? 75 : 95))
              return 1;
...
#define FLIP_BIT(_ar, _b) do { \
  ...
  stage_short = "int16";
  stage_cur   = 0;
  stage_max   = (len - 1) * (sizeof(interesting_16) >> 1);
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      switch (UR(16)) {
        ...
        case 4:
          /* Set the low 4 bits of a byte to a random value. */
          out_buf[UR(temp_len)] ^= 1 + UR(ARITH_MAX);
          break;

        case 5:
          /* Just set a random byte to a random value. Because, why not. */
          out_buf[UR(temp_len)] = UR(128);
          break;
        ...
        case 10:
          ...
          /* Tail */
          memcpy(out_buf + clone_to + clone_len, out_buf + clone_to,
                          temp_len - clone_to);

          if (UR(4))
            memcpy(out_buf + clone_to, out_buf + clone_from, clone_len);
          else
            memset(out_buf + clone_to, UR(128), clone_len);
        ...
        case 11: {
            ...
            } else memset(out_buf + copy_to, UR(128), copy_len);
        ...
        case 12 ... 15: {
            ...
            u8 extra[HAVOC_BLK_LARGE] = { '\033', '[' };
            u8 extra_len = 2;
            while (!UR(4))
                    extra[extra_len++] = 60 + UR(4);
            while (UR(3))
                    extra[extra_len++] = 48 + UR(12);
            while (!UR(5))
                    extra[extra_len++] = 32 + UR(16);
            extra[extra_len++] = 64 + UR(63);

            u32 insert_at = UR(temp_len);

            if (UR(2)) {
                memcpy(out_buf + insert_at + extra_len, out_buf + insert_at,
                                    temp_len - insert_at);
                temp_len += extra_len;
            }

            memcpy(out_buf + insert_at, extra, extra_len);
    ...
    if (queued_paths != havoc_queued) {
      stage_max = MIN(stage_max + HAVOC_CYCLES, HAVOC_CYCLES * HAVOC_MAX_MULT);
      havoc_queued = queued_paths;
    }
